...
//file imports
// we have c reated thibs folder inside the folder named breadcrumb ,and now we are exporting this function from that file inside lib folder to our this file
const buildBreadcrumb = require('./breadcrumb.js');

const buildMainContent = require('./mainContent.js');
const getMimeType = require('./getMimeType.js');

//static base bath: location of your static folder.i.e when a person will be goin to the home page of our app ,he wil be able to see the files available with the static folder.
const staticBasePath = path.join(__dirname, '..', 'static');
// when we are at home directory ,we are looking at static folder(directly),i.e what so ver will be there in this statstic folder ,all thos ethings will be shown directory on our hpomepage.
...
const respond = (request, response) => {
    ...
    let pathname = url.parse(request.url, true).pathname;
     // url.parse(request.url, true).pathname GIVES US THE PATHNAME GIVEN AT THE DOMAIN .
    //if favicon.ico stop
    if(pathname === '/favicon.ico'){
        return false;
    }
// our pathname may have different charecters with them ,but using decodeURIcomponent will decode the pathname easily into simpl charecters wihtout anuy kind of special symbols.
    ...
    if(!fs.existsSync(fullStaticPath)){
        ...
        // we will send 404 file not found if the file wont be found.
    ...
    if(stats.isDirectory()){
        ...
        //build table rows (main_content)f
...
// we are exporting a respond function that we have createReadStream
// on line number 17th ,to the app.js where we hhave required respond.js file...
